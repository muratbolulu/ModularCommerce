...
namespace Catalog.Api.Features.Catalog.Sync;
    ...
    private async Task ConsumeCreatedAsync(IChannel channel, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (@event is not null)
            {
                ...
                await sender.Send(new SyncProductCreatedCommand(@event), cancellationToken);
            }
    ...
    private async Task ConsumeUpdatedAsync(IChannel channel, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (@event is not null)
            {
                using var scope = _scopeFactory.CreateScope();
                var sender = scope.ServiceProvider.GetRequiredService<ISender>();
                await sender.Send(new SyncProductUpdatedCommand(@event), cancellationToken);
            }
